    ...
    <el-main>
      ...
      <el-row >
        <el-col :span="6" :offset="9" align="center">
          ...
          <el-button
            @click="getDetails($route.params.roleGroupId)">
            取消
          </el-button>
        </el-col>
      </el-row>
...
export default {
  ...
  mounted() {
    this.getDetails(this.$route.params.roleGroupId)
    this.getTreeList()
  ...
  methods: {
    ...
    async getDetails(id) {
      ...
      try {
        ...
        const r = await getRoleGroupDetails(params)
        this.form['groupName'] = r['data']['groupName']
        if (r['data']['roleList']) {
          this.form['roleGroupList'] = r['data']['roleList'].map(i => i['roleId'])
        }
        this.roleGroupParentId = r['data']['paths']
        this.form['description'] = r['data']['description']
        this.form['groupId'] = id
        this.baseRoleHandleCheckChange(this.form['roleGroupList'])
      } catch (e) {
        console.log(e)
      } finally {
        this.isLoading = false
